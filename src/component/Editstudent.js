    ...
    const formik = useFormik({
        ...
        onSubmit(values) {
           ...
           handleSave(values)
            ...
            mobile:yup.string().matches(/^\d{10}$/,"mobile number invalid").required('required'),
            degree:yup.string(),
            dept:yup.string(),
        }),
...
let handleSave=async(data)=>{
    await axios.put('https://614eacd3b4f6d30017b48344.mockapi.io/user/'+params.id,data)
navigate('/all-students')
}

    //to get the data of specific ID
    let getData = async()=>{
        let res=await axios.get('https://614eacd3b4f6d30017b48344.mockapi.io/user/'+params.id)
